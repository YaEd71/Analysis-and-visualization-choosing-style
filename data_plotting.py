...
def create_and_save_plot(data, ticker, period, filename=None, plot_type='all', style='default'):
    """
    ...
    """
    try:
        # Список доступных встроенных стилей matplotlib
        available_styles = [
            'default', 'classic', 'ggplot', 'bmh', 'fivethirtyeight',
            'seaborn', 'tableau-colorblind10'
        ]

        # Проверка и установка корректного стиля
        if style not in available_styles:
            print(f"Стиль {style} не найден. Будет использован стиль 'default'.")
            style = 'default'
        ...
        # Название графика с указанием выбранного стиля
        fig.suptitle(f"{ticker} Stock Analysis (Style: {style})", fontsize=16)
        ...
        # Сохранение графика
        if filename is None:
            filename = f"{ticker}_{period}_stock_indicators_chart_{style}.png"
